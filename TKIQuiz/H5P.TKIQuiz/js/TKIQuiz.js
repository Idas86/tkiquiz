  ...
  function TKIQuiz(params, id) {
    ...
    */
    ...
    /**
      ...
      @param {TKIQuiz} quiz A personality quiz instance
    ...
    */
    ...
    /**
      ...
      @param {TKIQuiz} quiz A TKIQuiz instance
      @param {Object} data The params received from H5P
    ...
    function attach($container) {
      ...
      $.when.apply(null, loadingImages).done(function () {
        ...
        if (animation && params.resultScreen.animation === 'wheel') {
          ...
          self.wheel = new TKIQuiz.WheelAnimation(
            self,
            self.personalities,
            canvas.width,
            canvas.height,
    ...
    */
    ...
    /**
    Imposta la schermata finale mostrando una tabella con le colonne:
    "Mode", "Raw Score" e "Percentile Score".
    Per ogni personalità, la riga mostrerà il nome, il conteggio (Raw Score)
    e una barra animata (Percentile Score) in base alla percentuale ottenuta.
    */
    self.setResultsTable = function () {
      // Totale delle risposte (assumiamo che self.answered sia il numero totale di risposte)
      var total = self.answered;

      // Definisci un array di colori per le righe (personalizza i valori come preferisci)
      var rowColors = ['#e75621', '#faaa15', '#33009b', '#a60495', '#e10046'];

      // Ordina le personalità in ordine decrescente in base al count
      var sortedPersonalities = self.personalities.slice().sort(function(a, b) {
        return b.count - a.count;
      });

      // Crea la struttura della tabella
      var $table = $('<table>', { 'class': classes('results-table') });
      
      // Intestazione della tabella
      var $thead = $('<thead>');
      var $headerRow = $('<tr>');
      $headerRow.append($('<th>', { 'html': '<span>Mode</span>' }));
      $headerRow.append($('<th>', { 'html': '<span>Raw Score</span>' }));
      $headerRow.append($('<th>', { 'html': '<span>Percentile Score</span><div><span>Low</span><span class="medium">Medium</span><span>High</span></div>' }));
      $thead.append($headerRow);
      $table.append($thead);
      
      // Corpo della tabella
      var $tbody = $('<tbody>');
      sortedPersonalities.forEach(function(personality, index) {
        
        var maxScore = 12; // Numero totale delle possibilità massime
        var rawPercentage = Math.round((personality.count / maxScore) * 100);

        var $row = $('<tr>');
        
        // Definisci il colore da usare per questa riga
        var color = rowColors[index % rowColors.length];
        
        // Colonna "Mode": il nome della personalità con background color
        var $tdName = $('<td>', { 'html': personality.name });
        $tdName.css('background-color', color);
        $row.append($tdName);
        
        // Colonna "Raw Score": il conteggio delle risposte
        $row.append($('<td>', { 'html': personality.count }));
        
        // Colonna "Percentile Score": contenitore per la barra animata
        var $barContainer = $('<div>', { 'class': classes('percentile-bar-container') });
        var $bar = $('<div>', { 'class': classes('percentile-bar') });
        
        // Imposta larghezza iniziale a 0 e applica il colore al background della barra
        $bar.css({
          'width': '0%',
        ...
        $row.append($('<td>').append($barContainer));
        
        $tbody.append($row);
        
        // Avvia l'animazione della barra dopo un breve timeout
        setTimeout(function() {
          $bar.css('width', rawPercentage + '%');
          // Imposta il testo centrato nella barra con la percentuale e la classificazione
          $bar.text(rawPercentage + '%');
        }, 100);
      });      
    
      $table.append($tbody);
      
      // Sostituisci il contenuto del wrapper dei risultati con la tabella creata
      self.$resultWrapper.empty().append($table);
    };
